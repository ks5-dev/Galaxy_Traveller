...
#            game_dir         build            exe.linux-x86_64-3.9        lib
#Sprite loading
        ...
        def update(self,x = 0,y = 0):
            ...
            if x-em1_range <= self.pos.x <= x+em1_range or y-em1_range <= self.pos.y <= y+em1_range :
                ...
                if now - self.last >= self.cooldown:
                    self.last = now
                    self.groups()[0].add(Projectile2(self.rect.center, self.direction.normalize()))
                    self.groups()[0].add(Projectile2(self.rect.center, self.direction.normalize().rotate(90)))
                    self.groups()[0].add(Projectile2(self.rect.center, self.direction.normalize().rotate(180)))
                ...
                if e.type == pygame.KEYDOWN:
                    if e.key == pygame.K_SPACE:
                        if not self.multishot and not self.ricochet :
                            ...
                            self.multishot = False
                            self.groups()[0].add(Projectile(self.rect.center, self.direction.normalize(),ricochet=True))
                        elif self.multishot :
                            self.ricochet = False
                            self.groups()[0].add(Projectile(self.rect.center, self.direction.normalize()))
                            direction_1 = self.direction.normalize().rotate(30)
                            direction_2 = self.direction.normalize().rotate(330)
                            self.groups()[0].add(Projectile(self.rect.center, direction_1))
                            self.groups()[0].add(Projectile(self.rect.center, direction_2))
